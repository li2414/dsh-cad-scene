...
function makeTool(ctx, cfg) {
  return {
    ...
    description: 'Parse a CAD drawing of a logistics warehouse into a structured scene JSON for a Three.js 3D scene. DXF files are parsed directly with dxf-parser; DWG files are converted to DXF first via an external converter (LibreDWG dwg2dxf or ODA File Converter; auto-detected, or pinned with the row config dwgConverter). Returns { racks, aisles, zones, agvs, entities, layers, meta }: entities holds every parsed entity (LWPOLYLINE/POLYLINE/LINE vertices, ARC/CIRCLE center+radius with angles in radians, TEXT/MTEXT content, INSERT block references, etc.) with its layer; layers holds the layer table (name, color, visibility); racks/aisles/zones/agvs are the subsets classified by layer-name keywords (rack/货架 -> racks, aisle/通道 -> aisles, zone/区域 -> zones, agv/小车 -> agvs); meta lists block definitions and parse stats.',
    parameters: {
      type: 'object',
      properties: {
    ...
    output: {
      schema: {
        ...
        required: ['racks', 'aisles', 'zones', 'agvs', 'entities', 'layers', 'meta'],
        properties: {
    ...
    async execute(args) {
      ...
      return parseDxfToScene(dxfText, procPath, format)
    },
  }
}
...
function apply(ctx, config) {
  ...
  if (webServer === undefined) {
    console.warn('[dsh-cad-scene] webServer unavailable; DWG panel route not registered')
    return
  }
  ctx.effect(() => webServer.register({ kind: 'exact', path: ROUTE_PATH, handler: (req, res) => handleParse(req, res, cfg) }), 'dsh-cad-scene: parse route')
}
