...
// ── AI device recognition (Host `llm` service) ─────────────────────────────

const IDENTIFY_ROUTE = '/api/cad-scene-builder/identify'
const IDENTIFY_TIMEOUT_MS = 45000

const IDENTIFY_SYSTEM_PROMPT = [
  'You are a logistics-warehouse CAD device recognition expert.',
  'Input: a device digest parsed from a CAD drawing (device groups per layer) plus an enterprise equipment catalog (model_library and device_types).',
  'Task: judge the equipment type of every group and bind it to a catalog model; judge the individual anomaly devices separately.',
  'Reply with a single JSON object and nothing else, with exactly these keys:',
  '{"group_bindings": [{"key","type","model","confidence","evidence"}], "devices": [{"device_id","type","model","confidence","evidence"}], "unmatched": [{"device_id","key","reason"}]}',
  '- group_bindings: exactly one entry per digest.groups[].key (the layer name). type MUST be one of the catalog.device_types keys. model MUST be one of the catalog.model_library keys.',
  '- devices: per-device overrides ONLY for digest.anomalies entries (device_id must match).',
  '- confidence: 0..1. evidence: short strings quoting the digest (texts, sizes, geometry, layer names).',
  '- unmatched: anything you cannot judge; never drop devices silently.',
].join('\n')

function identifyUserMessage(text) {
  return {
    id: 'identify-' + Date.now().toString(36) + '-' + Math.floor(Math.random() * 1e9).toString(36),
    role: 'user',
...
function extractJsonReply(reply) {
  const withoutFences = String(reply || '').replace(/```[a-zA-Z]*\s*/g, '')
...
async function handleIdentify(req, res, deps) {
  if (!isLoopbackRequest(req)) return writeJson(res, 403, { error: 'forbidden' })
  if (req.method !== 'POST') return writeJson(res, 405, { error: 'method-not-allowed' })
  const llm = deps.getLlm()
  if (!llm) return writeJson(res, 503, { ok: false, code: 'no-model', error: 'this deployment serves no llm service' })
  let payload
  try {
    payload = JSON.parse((await readBody(req)).toString('utf8'))
  } catch (error) {
    return writeJson(res, 400, { ok: false, code: 'parse-failed', error: 'request body is not valid JSON' })
  }
  const digest = payload && payload.digest
  if (!digest || typeof digest !== 'object') {
    return writeJson(res, 400, { ok: false, code: 'parse-failed', error: 'missing digest' })
  }
  // model route: request override -> row config -> deployment default model
  let route = splitModelRoute(payload.model) || splitModelRoute(deps.cfg.model)
...
function apply(ctx, config) {
  ...
  ctx.inject(['webServer'], (webCtx) => {
    const deps = {
      ...
      getLlm: () => {
        const l = ctx.get('llm')
        return l && typeof l.stream === 'function' ? l : null
      },
      defaultRoute: () => {
        try {
          const m = ctx.get('agentDefaultModel')
          const sel = m && typeof m.currentSelection === 'function' ? m.currentSelection() : null
          if (!sel) return null
          if (typeof sel === 'string') return splitModelRoute(sel)
          if (sel.provider && sel.model) return { provider: String(sel.provider), model: String(sel.model) }
          return splitModelRoute(sel.route || sel.model)
        } catch (e) {
          return null
        }
      },
    ...
    webCtx.effect(() => {
      ...
      const offIdentify = webCtx.webServer.register({ kind: 'exact', path: IDENTIFY_ROUTE, handler: (req, res) => handleIdentify(req, res, deps) })
      return () => {
        offParse()
        offIdentify()
      }
    }, 'dsh-cad-scene: parse + identify routes')
  })
}
